...
FORBIDDEN_SUFFIXES = {".wav", ".mp3", ".m4a", ".flac", ".ogg", ".opus", ".mp4", ".mov",
                      ".png", ".jpg", ".jpeg", ".pdf", ".epub", ".safetensors", ".gguf",
                      ".pt", ".pth", ".bin", ".sqlite", ".db", ".jsonl", ".ipynb"}
FORBIDDEN_PARTS = {"private", "datasets", "outputs", "runs", "models", "checkpoints", ".venv"}
PATTERNS = {
    "private absolute path": re.compile(r"/(?:Users|home)/[A-Za-z0-9_.-]+/"),
    "private IPv4 address": re.compile(r"\b(?:192\.168\.\d{1,3}\.\d{1,3}|10\.\d{1,3}\.\d{1,3}\.\d{1,3})\b"),
    "private key": re.compile(r"-----BEGIN [A-Z ]*PRIVATE KEY-----"),
    "provider credential": re.compile(r"\b(?:sk-(?:proj-)?[A-Za-z0-9_-]{24,}|gsk_[A-Za-z0-9]{24,}|gh[pousr]_[A-Za-z0-9]{25,}|AIza[A-Za-z0-9_-]{30,})\b"),
    "personal email": re.compile(r"\b[A-Za-z0-9._%+-]+@(?:gmail|hotmail|outlook|yahoo)\.com\b", re.I),
}
...
def check(path, payload, terms):
    problems = []
    location = Path(path)
    if location.suffix.lower() in FORBIDDEN_SUFFIXES or set(location.parts) & FORBIDDEN_PARTS:
        problems.append("private/generated file type or directory")
    if location.name == ".env" or (location.name.startswith(".env.") and location.name != ".env.example"):
...
def main():
    ...
    terms = [s.strip().casefold() for s in Path(args.terms_file).read_text().splitlines() if s.strip()] if args.terms_file else []
    failures, count = [], 0
    if args.history:
        objects = git("rev-list", "--objects", "--branches", "--tags").decode().splitlines()
        records = []
        for item in objects:
    ...
    print(json.dumps({"checked": count, "findings": {s: failures.count(s) for s in sorted(set(failures))}}))
