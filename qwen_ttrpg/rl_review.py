...
def render(paths, output):
    ...
    for index, key in enumerate(grouped[0]):
        ...
        for report, rows in zip(reports, grouped):
            ...
            sections.append(f'<section><h3>{esc(report["candidate"])}</h3><p>{"Pass" if grade["success"] else "Fail"} · {grade["tool_calls"]} calls · {grade["seconds"]:.2f}s</p><pre>{esc(json.dumps(row["final"],indent=2))}</pre><details><summary>Decisions, tool results, and grading</summary><pre>{esc(json.dumps({"assessment":grade,"trace":row["trace"]},indent=2))}</pre></details></section>')
    ...
    for label, stats in summary.items():
        ...
        header += '<tr>' + ''.join('<td>' + esc(value) + '</td>' for value in [
            label, f'{overall.get("successes", "—")}/{overall.get("cases", "—")}',
...
def main():
    ...
    print(json.dumps(render(args.reports,args.output),indent=2))
